...
export default function OrbundTrialPage() {
  ...
  useEffect(() => {
    Promise.all([
      lmsApi.trialConfig()
        ...
        .catch(() => setConfigErr('Failed to load form options. Please refresh.')),
      orbundProxy.getSessionId()
        .then((data: { sessionId?: string }) => {
          const sid = data?.sessionId || '';
          sessionIdRef.current = sid;
          localStorage.setItem('orbund_session_id', sid);
        })
        .catch(() => {}),
      lmsApi.me()
        .then((user: { name?: string; email?: string; phone?: string } | null) => {
          if (user) {
            setName(user.name || '');
            setEmail(user.email || '');
            setPhone(user.phone || '');
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    localStorage.setItem('orbund_trial_reg', JSON.stringify(reg));

    router.push('/orbund_trial/classes');
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-8">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-gray-900">Enroll in a Class</h1>
          <p className="text-gray-500 mt-1">Step 1 of 6 — Tell us about yourself and your child</p>
        </div>

        <div className="w-full bg-gray-200 rounded-full h-2 mb-8">
          <div className="bg-indigo-600 h-2 rounded-full" style={{ width: '17%' }} />
        </div>
