...
export default function BillingPage() {
  ...
  const sessionId = typeof window !== 'undefined' ? localStorage.getItem('orbund_session_id') || '' : '';

  useEffect(() => {
    (async () => {
      try {
        const [billingData, stateData] = await Promise.allSettled([
          orbund.getBillingInfo(sessionId),
          orbund.getStates(sessionId, 'CA'),
        ]);
  ...
  async function handleCountryChange(val: string) {
    ...
    try {
      const data = await orbund.getStates(sessionId, val);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const orbundRes = await orbund.processPayment(sessionId, { ...form, country });

      if (orbundRes?.error) {
        setError(orbundRes.error?.message || 'Payment failed. Please check your card details.');
        setSubmitting(false);
        return;
      }

      // Save payment to our DB
      const enrollmentId = localStorage.getItem('lms_enrollment_id');
      if (enrollmentId) {
        const dueObj = sessionStorage.getItem('dueObj');
        const amount = dueObj ? parseFloat(JSON.parse(dueObj).dueNow?.replace(/[^0-9.]/g, '') || '0') : 0;
        await lmsApi.saveTrialPayment({
          enrollment_id: Number(enrollmentId),
          amount,
          orbund_transaction_id: orbundRes?.transactionId || '',
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-8">
        <div className="text-center mb-6">
          ...
          <p className="text-gray-500 mt-1">Step 6 of 7 — Enter your payment details</p>
        </div>
        ...
        {loading ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex gap-3 pt-2">
              <button
                type="button" onClick={() => router.push('/trial/checkout')}
